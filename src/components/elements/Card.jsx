...
export default function Card({title, desc, color}) {
  ...
  return (
    <section className={` min-h-32 ${color} p-8 motion-preset-slide-up flex md:flex-col-reverse md:gap-7 hover:scale-105 hover:shadow-sm transition-all cursor-pointer  md:rounded-lg justify-between items-center rounded-e-lg me-3 md:m-0 my-3`}>
        <div className='basis-2/3 md:basis-1'>
        <h3 className='font-medium text-xl md:text-4xl md:text-center md:my-4 '>{title}</h3>
        <p className='text-accent md:text-center md:text-lg md:pb-7 md:mx-[10%] me-5'>{desc}</p>
        </div>
